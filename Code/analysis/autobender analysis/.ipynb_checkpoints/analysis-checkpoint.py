...
class bender_class:
    ...
    def plot_mech_model_data(self, thick, l_ch, l_sam, area, res, scatter=False,
        ...
        r2_model2data_cd = 1 - (ss_res / ss_tot)

        # --- Compute R² as Variance Accounted For (VAF), not CoD ---
        y = np.asarray(self.data['ADC Value'], dtype=float)
        yhat = np.asarray(model_at_data, dtype=float)
        ...
        resid = y - yhat
        var_y = np.var(y, ddof=0)  # population variance (consistent with VAF usage)
        var_resid = np.var(resid, ddof=0)

        r2_model2data = 1.0 - (var_resid / var_y) if var_y > 0 else np.nan
